...
## 1. 文档信息
- 版本：v0.8
- 状态：S20（编辑器 Pipeline + 蓝图滚动闭环 + 设置页 AI 策略 + 派生审阅提示）
- 最后更新：2026-04-30
...
- 8 步蓝图编辑、保存、完成标记、重置。
- 支持 AI 生成步骤建议并应用。
- 支持窗口规划区（`windowPlanningHorizon`、卷进度、章节目标）与摘要回报区（关键变量变化、漂移警告、晋升/状态计数）。
- 已完成步骤可触发“确认并晋升”。
- 当晋升目标为 `chapter-plan` 时，支持章节选择策略：
  1. 用户显式选择；
  2. 当前活动章节；
  3. 首个可规划章节（优先未完成）；
  4. 无可选章节时给出可见提示，不静默失败。
- 数据源：`blueprintApi`。
...
- AI 策略：
  - 项目级 `AiStrategyProfile` 配置面板（工作流栈、审查严格度、能力包、自动化档位、状态写入策略、Continuity Pack 深度、章节生成模式、窗口规划地平线）。
...
## 6. 当前已知 UI 风险点
- 后端仍保留少量 compatibility-only 命令，若被外部旧入口调用，可能造成链路认知混淆。
