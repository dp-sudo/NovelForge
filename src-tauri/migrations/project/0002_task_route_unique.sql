-- Canonicalize project-level llm_task_routes.task_type values, deduplicate legacy aliases
-- per project, then enforce one route per (project_id, canonical task_type).
    ...
    ROW_NUMBER() OVER (
      PARTITION BY
        ...
        CASE task_type
          ...
          ELSE task_type
        END
      ORDER BY priority DESC, updated_at DESC, created_at DESC, rowid DESC
    ) AS rn
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_llm_task_routes_project_task_type
ON llm_task_routes(project_id, task_type);
