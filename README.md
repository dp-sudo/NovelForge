...
- 本地目录协议、存储协议、导出协议变化。
- AI 接入策略、模型配置字段变化。
- 任务路由 canonical 映射、默认路由自动补全、重试链行为变化。
- AI Pipeline 事件协议（`ai:pipeline:event`）与阶段定义变化。
- 结构化草案池（`structured_draft_*`）字段或人工确认落库协议变化。
...
## 6. 文档验收清单（最小）
1. 链接路径可打开，无失效引用。
2. 文档中的命令名、字段名、错误码与当前代码一致。
3. 文档描述的是“已实现行为”，不是计划性描述。
4. 事件协议与 UI 实际消费逻辑一致（尤其是 AI Pipeline 流事件）。
5. 数据库表结构描述与迁移文件/兼容补列逻辑一致。

## 7. 版本记录
- 2026-05-01（S21）：全面文档维护，同步迁移文件 `0007_blueprint_certainty_zones`、确认 AppState 服务组成、验证命令注册完整性、更新所有核心维护文档至最新代码基线。
- 2026-04-28（S18）：同步 AI Pipeline v1（`run_ai_task_pipeline`/`cancel_ai_task_pipeline`）、编辑器 9 按钮任务映射、`ai:pipeline:event` 协议、结构化草案池（关系/戏份/场景）人工确认落库闭环、写作风格配置与技能管理文档。
