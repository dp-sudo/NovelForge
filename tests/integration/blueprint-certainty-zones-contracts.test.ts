...
test("确定性分区契约：orchestrator 具备 DTO 优先并兼容文本回退", async () => {
  ...
  assert.match(orchestrator, /PIPELINE_FREEZE_CONFLICT/);
});

test("确定性分区契约：BlueprintService 校验跨分区重叠并返回专用错误码", async () => {
  const service = await readRepoFile("src-tauri/src/services/blueprint_service.rs");
  assert.match(service, /validate_certainty_zone_conflicts/);
  assert.match(service, /BLUEPRINT_CERTAINTY_ZONES_OVERLAP/);
});
