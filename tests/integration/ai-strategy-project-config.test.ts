...
test("AI 策略契约：前端默认策略与后端默认能力栈保持一致", async () => {
  const aiTypes = await readRepoFile("src/types/ai.ts");
  const projectService = await readRepoFile("src-tauri/src/services/project_service.rs");
  ...
  for (const expected of [
    ...
    assert.match(aiTypes, new RegExp(expected.replaceAll(".", "\\.")));
    assert.match(projectService, new RegExp(expected.replaceAll(".", "\\.")));
  }
});
