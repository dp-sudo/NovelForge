...
test("问题4契约验证：compatibility-only 命令仍注册且文档标记为 deprecated", async () => {
  const libRs = await readRepoFile("src-tauri/src/lib.rs");
  ...
  for (const command of deprecatedCommands) {
    assert.match(libRs, new RegExp(`commands::[a-z_]+::${command}`));
    assert.match(apiDoc, new RegExp(`${command}[\\s\\S]*compatibility-only`, "i"));
  }
...
test("问题3文档同步：legacy AI 命令标记为已移除，主链路统一到 pipeline", async () => {
  const runtimeDoc = await readRepoFile("docs/runtime/runtime-process-spec.md");
  const apiDoc = await readRepoFile("docs/api/api-integration-spec.md");

  assert.match(runtimeDoc, /legacy AI 命令 .*已从当前代码命令面移除/);
  assert.match(apiDoc, /legacy AI 命令 .*已从当前命令面移除/);
