...
test("问题2契约验证：run_ai_task_pipeline payload 形状保持前后端一致", async () => {
  ...
  assert.doesNotMatch(pipelineApi, /resolvePersistPolicy/);
  assert.doesNotMatch(pipelineApi, /autoPersist:/);
  assert.match(pipelineApi, /persistMode:\s*input\.persistMode/);
  assert.match(pipelineApi, /automationTier:\s*input\.automationTier/);
  assert.match(
    pipelineApi,
